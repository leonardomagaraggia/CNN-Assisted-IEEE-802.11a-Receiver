classdef LOS_CNN < handle
    ...
    methods
        ...
        function Xn = normalizeInputBatch(obj, X, carrierIdx)
            ...
            Xn = X;
            iqPairs = [1 2; 3 4; 5 6];
            for k = 1:size(iqPairs, 1)
                pairIdx = iqPairs(k, :);
                if pairIdx(end) > obj.NumInputChannels
                    continue;
                end

                Xscale = X(pairIdx, carrierIdx, :);
                scale = sqrt(squeeze(mean(mean(Xscale.^2, 1), 2)) + single(1e-8));
                Xn(pairIdx, :, :) = X(pairIdx, :, :) ./ reshape(scale, 1, 1, []);
            end
        end
    ...
    methods (Static)
        function version = architectureVersion()
            version = "carrier_resnet_v6_ieee80211a_ltf_delay_regularized";
        end
        ...
        function loss = crossEntropyLoss(dlY, dlT, carrierIdx)
            if nargin >= 3 && ~isempty(carrierIdx)
                carrierIdx = double(carrierIdx(:)).';
                dlY = LOS_CNN.selectCarrierDimension(dlY, carrierIdx);
                dlT = LOS_CNN.selectCarrierDimension(dlT, carrierIdx);
            end

            carrierDim = LOS_CNN.dimensionByLabel(dlT, 'T', 2);
            batchDim = LOS_CNN.dimensionByLabel(dlT, 'B', 3);
            lossTerms = -dlT .* log(dlY + single(1e-8));
            loss = sum(lossTerms, 'all') / (size(dlT, carrierDim) * size(dlT, batchDim));
        end

        function [loss, gradients] = modelGradients(net, dlX, dlT, carrierIdx)
            if nargin < 4
                carrierIdx = [];
            end

            dlY = forward(net, dlX);
            loss = LOS_CNN.crossEntropyLoss(dlY, dlT, carrierIdx);
            gradients = dlgradient(loss, net.Learnables);
        end

        function dlX = selectCarrierDimension(dlX, carrierIdx)
            carrierDim = LOS_CNN.dimensionByLabel(dlX, 'T', 2);
